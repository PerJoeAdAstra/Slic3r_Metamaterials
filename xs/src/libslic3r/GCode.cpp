...
GCode::extrude(ExtrusionLoop loop, std::string description, double speed)
{
    ...
    //if(config.texture_test){
    //  for (ExtrusionPaths::const_iterator path = paths.begin(); path != paths.end(); ++path)  //TODO: Change this!
    //  //Do nothing!
    ...
    if (paths.back().role == erExternalPerimeter && this->layer != NULL && this->config.perimeters > 1) {
        ...
        // the side depends on the original winding order of the polygon (left for contours, right for holes)  // TODO: Movement order for holes/Contours
...
GCode::needs_retraction(const Polyline &travel, ExtrusionRole role)
{
    ...
    if(role == erInternalInfill){
      if(this->config.force_infill_retraction){
        return true;
      }
    }
