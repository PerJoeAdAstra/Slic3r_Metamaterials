        ...
        if (loop_number >= 0) {  // no loops = -1
            ...
            for (int i = 0; i <= loop_number+1; ++i) {  // outer loop is 0
                ...
                if (i == 0) {
                    ...
                    // ext_width/2 + ext_spacing/2 + spacing/2 + width/2             ***TODO*** This is where the perimeter modification stuff should go!
                    if (this->config->thin_walls) {                                // Check spiral vase aswell just in case
                        offsets = offset2(
                            last,
                ...
                } else {                                     //For every loop apart from the first
                ...
                for (Polygons::const_iterator polygon = offsets.begin(); polygon != offsets.end(); ++polygon) {
                    PerimeterGeneratorLoop loop(*polygon, i);                       //Creation of PerimeterLoop
                    loop.is_contour = polygon->is_counter_clockwise();              //Checks for clockwise/anti for loops/ holes
                    if (loop.is_contour) {
                ...
                for (int i = 0; i < (int)holes_d.size(); ++i) {
                    ...
                    for (int t = d+1; t <= loop_number; ++t) {
                        for (int j = 0; j < (int)holes[t].size(); ++j) {
                            ...
                            if (candidate_parent.polygon.contains(loop.polygon.first_point())) {     //TODO figure this out:
                                candidate_parent.children.push_back(loop);                           // What is candidate parent - a hole
                    ...
                    for (int t = loop_number; t >= 0; --t) {
                        for (int j = 0; j < (int)contours[t].size(); ++j) {
                            PerimeterGeneratorLoop &candidate_parent = contours[t][j];                // otherwise find which contour the hole fits in
                ...
                for (int i = 0; i < (int)contours_d.size(); ++i) {
                    ...
                    for (int t = d-1; t >= 0; --t) {
                        for (size_t j = 0; j < contours[t].size(); ++j) {
                            ...
                            if (candidate_parent.polygon.contains(loop.polygon.first_point())) {   // Sort out all contours
                                candidate_parent.children.push_back(loop);
